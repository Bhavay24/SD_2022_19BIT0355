...
Original file is located at
    https://colab.research.google.com/drive/1qCv14MD56snF2GR1gE5UxpxZU8ZYHsEa
"""
...
pic_dir = "/mnt/DATA/Dropbox/Courses/Python/Project/Pictures"
...
class ChessBoard:
	...
	def __str__(self): 
		string = ''
		self.symbolboard = self.draw_board(self.objectboard.T) # Transpose is necessary to make the board as we are accustomed to
	...
	def show_board(self, symbolboard):
		window = tk.Tk()
		canvas = tk.Canvas(width = board_width, height = board_height)
		canvas.pack()
		images = {}
		for image_file_name in DICT:
			...
			if not os.path.exists(f):
				...
				exit(-1)
			images[image_file_name]= tk.PhotoImage(file=f)
		for j in range(8):
			y = j * tile_width
			for i in range(8):
				x = i * tile_width
				if (i % 2 ==  ((1 + j) % 2)):  
					tile = images['black_tile']
				else:
					tile = images['white_tile']
				canvas.create_image(x, y, anchor = tk.NW,  image=tile)
		for j in (range(8)): 
			for i in (range(8)):
...
print(chessboard)
chessboard.show_board(chessboard.symbolboard)
# Already mate here
